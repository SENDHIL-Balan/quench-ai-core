...
/**
 * The reference scene is intentionally quiet: a near-black central space,
 * atmospheric colour around the perimeter, and an Earth-like horizon that
 * only occupies the lower-left edge.
 */
export function CosmicBackground() {
  ...
  useEffect(() => {
    ...
    const drawEdgeGlow = () => {
      ...
      const topRight = ctx.createRadialGradient(width * 0.92, height * 0.02, 0, width * 0.92, height * 0.02, width * 0.42);
      ...
      const lowerLeft = ctx.createRadialGradient(0, height, 0, 0, height, Math.max(width, height) * 0.78);
