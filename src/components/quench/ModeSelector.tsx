import { MessageSquare, Search, PenLine, BarChart3, Code2, Image as ImageIcon } from "lucide-react";
...
export function ModeSelector({
  ...
  return (
    <div
      className={cn(
        "w-full shrink-0 relative z-20 py-1 flex items-center justify-start sm:justify-center",
        className,
      )}
    >
      <div className="flex items-center gap-2 overflow-x-auto max-w-full px-2 py-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:flex-wrap sm:justify-center">
        {VISIBLE_MODES.map((id) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex shrink-0 items-center gap-2 rounded-full border px-3.5 sm:px-4 py-2 text-xs sm:text-sm font-medium transition-all cursor-pointer min-h-[40px]",
                active
